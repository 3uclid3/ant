...
struct fixture
{
    ...
    auto emplace_combinations() -> void
    {
        if constexpr (I == Size)
        {
            ...
            {
                auto _ = _catalog.ensure_of(component_bitset_of<Cs...>());
            }
        }
        else
        {
...
TEST_CASE_FIXTURE(fixture, "catalog::ensure_of: create and retrieve table index matching components")
{
    ...
    CHECK(_catalog.empty());

    auto first_idx = _catalog.ensure_of(components);
    auto second_idx = _catalog.ensure_of(components);

    CHECK_EQ(first_idx, second_idx);
    CHECK_EQ(_catalog.size(), 1u);

    const auto& table = _catalog.at(first_idx);
...
TEST_CASE_FIXTURE(fixture, "catalog::index_of: returns npos for non-existing table")
{
    ...
    CHECK_EQ(_catalog.index_of(components), _catalog.npos);
}
...
TEST_CASE_FIXTURE(fixture, "catalog::for_each: match tables by components")
{
    ...
    _catalog.for_each(required, [&matched_tables](std::size_t idx, const auto& table [[maybe_unused]]) {
        matched_tables.push_back(idx);
    });

    std::vector<std::size_t> expected_tables{
        _catalog.index_of(required),
        _catalog.index_of(component_bitset_of<testing::component<0>, testing::component<1>, testing::component<2>>())};
...
TEST_CASE_FIXTURE(fixture, "catalog::for_each: required none matches all tables")
{
    ...
    std::vector<std::size_t> expected_tables{
        _catalog.index_of(component_bitset_of<testing::component<0>>()),
        _catalog.index_of(component_bitset_of<testing::component<1>>()),
        _catalog.index_of(component_bitset_of<testing::component<0>, testing::component<1>>())};
    CHECK(std::ranges::is_permutation(matched_tables, expected_tables));
}
...
TEST_CASE_FIXTURE(fixture, "catalog::for_each: no matches when required component absent")
{
    ...
    bool found = false;
    _catalog.for_each(required, [&found](std::size_t idx [[maybe_unused]], const auto& table [[maybe_unused]]) {
        found = true;
    });
...
TEST_CASE_FIXTURE(fixture, "catalog::for_each: matches after capacity growth")
{
    ...
    _catalog.for_each(required, [&count](std::size_t idx [[maybe_unused]], const auto& table [[maybe_unused]]) {
        ++count;
    });
